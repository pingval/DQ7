...
- 主人公の名前が何度も表示される通常RTAと違って名前の文字数を気にする必要がないので、DQ7の主人公の名前を「ぁ」から○連打で入力できるため操作ミスの少ない「ああああ」へ変更。
- チェックサムが0になり(= 最後の4byteを塗らなくてよくなる)、かつ塗りやすそうなバイナリを[crchack](https://github.com/resilar/crchack)で調べ、以下の塗り方に変更。
```
Hex:
...
- アニメティカの選択時のSTARTボタンや無圧縮セーブ終了後お絵かき画面に戻るときの×ボタンが認識されない謎のロスが発生したものの、アニメティカ終了で2m切ったのは初めて。
- DQ7のメモカ抜きは無事成功し、ロードに多少ｇｄったものの無事2:50を切れた。
...
今回のプレイでは8マス6色のパターンのひとつを用いた。  
以下の9マス5色のパターンも見た感じよさそう。
